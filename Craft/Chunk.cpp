	...
	for (int x = 0; x < max_x; ++x)
	{
		for (int y = 0; y < max_y; ++y)
		{
			for (int z = 0; z < max_z; ++z)
			{
				...
				if (block_type == GRASS)
				{
					block_type_top = -GRASS_TOP;
					block_type_bottom = -DIRT;
				}
				...
				bool self_tp = block_type == GLASS || block_type == WATER || block_type == LEAF;
				...
				int x_next_tp = x_next == GLASS || x_next == WATER || x_next == LEAF;
				int y_next_tp = y_next == GLASS || y_next == WATER || y_next == LEAF;
				int z_next_tp = z_next == GLASS || z_next == WATER || z_next == LEAF;
				int x_prev_tp = x_prev == GLASS || x_prev == WATER || x_prev == LEAF;
				int y_prev_tp = y_prev == GLASS || y_prev == WATER || y_prev == LEAF;
				int z_prev_tp = z_prev == GLASS || z_prev == WATER || z_prev == LEAF;

				byte4* vertices = self_tp ? vertices_transparent : vertices_opaque;
				unsigned int &n_vertices = self_tp ? n_transparent : n_opaque;
				...
				// from -y
				if ((!y_prev || y_prev_tp) && !(self_tp && y_prev) || (block_type == WATER && !y_next))
				{
